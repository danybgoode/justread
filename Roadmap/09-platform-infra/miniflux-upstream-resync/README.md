...
- **Three existing starter-feed lists** — `internal/ui/user_onboarding.go`, `add_subscription.html`, `scripts/enhance_miniflux.js` — consolidated, not rewritten, in S3 *(corrected by D13: the third list was the landing signup's; `enhance_miniflux.js` holds a keyword map)*

## Architecture decisions — locked by the orchestrator (D1–D14)
...
2. S2 deploys twice, deliberately: `v2.3.3` first, live until its soak passed (D11 — evidence, not calendar), then `upstream/main`. Two
...
4. S3 deploys once, and is low-risk: it shrinks the fork's code (the subscribe-page delta 235 → 25 lines) while adding `feeds.json` and the sync workflow (D13).
...
`resync-hop1-v2.3.3` (S2.2, `e219eb3d`), `resync-hop2-main` (S2.3, `2ee92c9b`), `resync-s3` (S3, `0f4bbb68`),
`resync-done` (final, `804e6dfc` — the pin production runs at epic close). S3.3's weekly sync inherits the rule:
`accept` tags the replaced tip `pre-sync-<stamp>` before moving `panfleto`.
