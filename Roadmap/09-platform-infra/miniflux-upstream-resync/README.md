...
| **D5** | Cleanup/verification order for the `cspNonce` chain | **To lock** — confirm against the live tree that `add_subscription.html:351` is the only `.cspNonce` consumer outside `layout.html` before planning S3's removal order |
| **D6** | Whether the two new upstream migrations are safe against *this* database | **To lock** — read row counts in `entries`, `users`, `feeds` with `psql` before S2 deploys. Row counts decide what is safe; migration files alone don't |
| **D7** | Submodule pin recorded where, and by whom | **To lock** — the pre-rebase SHA goes in this README (see *Rollback*), not just in reflog |
...
**Pre-rebase submodule SHA:** `<to be recorded by the orchestrator at the start of S2 — D7>`
